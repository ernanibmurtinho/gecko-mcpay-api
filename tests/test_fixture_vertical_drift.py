"""S31-#54 / S32-#58 drift guard — every fixture's ``vertical`` must be a
member of the canonical :data:`gecko_core.knowledge.taxonomy.Vertical`
Literal.
...
S32-#58 expanded ``Vertical`` with ``lending`` / ``liquid_staking`` /
``perps`` so the trade-vertical fixtures can move off the ``unknown``
fallback they took in #54. This test auto-accepts the new values because
it reads :func:`typing.get_args` rather than a hard-coded set.
"""
    ...
    for suite_file in sorted(SUITE_DIR.glob("*.json")):
        ...
        for fixture in _iter_fixtures(suite):
            ...
            if vert not in VALID_VERTICALS:
                offenders.append(
                    f"{suite_file.name} fixture {fixture.get('id')!r} has vertical={vert!r}"
                )
